...
- **README 必须有 `## 功能描述`**：所有插件的 `README.md` 都必须在文档靠上位置提供一节 `## 功能描述`（纯文字、不加代码），说明插件的**中文名**与**功能**（显示/隐藏开关、按钮入口、弹窗/tab 行为等）。新增插件必须写；已有插件补上。后续改需求也同步改这一节，作为判断/对齐问题的依据。
- **联动描述带中英文**：README（尤其 `## 功能描述`）里凡是**提到与其它插件联动**的地方（如"设置 → 某功能包 → 某管理"、或"内容由某插件注入"），都要用 **「中文名（包名）」** 双写（例：*功能包（dsh-web-ui-FuncPackagePanel，DSH功能包）*、*dsh-tool-skill-mcp-panel（技能/MCP 能力插件）*），包名用反引号包裹，避免被误读。
- **README 功能描述里要有版本**：`## 功能描述` 内需含「版本」小节 = 【当前版本】行 + 【版本历史】表（版本号 / 变更要点）。**以后每次改版/发版，都在该表上方追加一行**并同步更新「当前版本」，保持 README 与实现一致。
- **markdown 超链接后紧跟中文标点会卷进 URL**：DSH 的 markdown 渲染器对 `[文字](url)` 后面**紧贴**中文全角标点（如 `（`、`、`、`）`）处理有缺陷，会把后缀一并解析进 href（我在 README 实际踩到：`[x](url)（v2.0.2）` 的 `（v2.0.2）` 被卷进 URL）。**规避**：在链接 `](url)` 后加一个空格再接中文标点，如 `[Fishquito7/dsh-skill-mcp-panel](https://...) （v2.0.2）`。
...
### 3.5 版本号前缀规则（新规则 — 用户 2026-09-06 决策，下次更新发版统一遵守）
...
**用户定的规则（保留原话）**：「如果版本号不能重复，那就直接在前面增加版本号的前缀如何。」→ **下次更新（发版）插件时，统一用新规则命名**。

**规则**：**所有 cheeco 插件发版统一用「前缀式」tag**：`v<插件id>-<版本号>`。`<插件id>` = 该插件在 `publish.mjs` `META` 里的 `id`（同时等于功能推荐清单条目 `id`）。
...
- `publish.mjs` 已统一：`getOrCreateRelease`/`refreshFeatureInstall` 均用 `v${meta.id}-${version}`；`dsh-client-ui-plugin-push` 的 `resolveDownloadUrl` 已按 `PREFIX_BY_FOLDER` 前缀拼接。
- **下次更新发版一律按此规则**；旧的无法前缀 tag（如 `v0.3.3`）保留不管，新发版统一前缀。
...
- [ ] 用户已重启工作台。
- [ ] 插件 `README.md` 含 `## 功能描述`（新增插件必写；改需求时已同步该节）。
