window.__ModuleLoader__.load({
  id: '@cheeco/dsh-client-ui-plugin-manager',
  factory: (require) => {
    var module = { exports: {} };
...
function StatusTabs({ t, counts, active, onChange, onRefresh, refreshDisabled }) {
  const tabs = [
  ...
  return import_react2.default.createElement(
    ...
    onRefresh ? import_react2.default.createElement(
      "button",
      { className: "pmgr-btn", style: { marginLeft: "auto" }, disabled: refreshDisabled, onClick: onRefresh },
      t("refresh")
    ) : null
  );
}
...
function PluginManagerTab(props) {
  ...
  const refresh = (silent) => {
    ...
    pmgr.list().then((data) => {
      ...
      if (p) {
        try {
          window.__dshCheecoProfile = p;
          window.dispatchEvent(new Event("dsh-cheeco-profile"));
        } catch {
        }
      }
  ...
  return import_react3.default.createElement(
    "div",
    { className: "pmgr-root" },
    // 状态筛选 Tabs（刷新在最右，不再单独显示页头标题）
    import_react3.default.createElement(StatusTabs, {
      t,
      counts,
      active: statusFilter,
...
function apply(ctx) {
  ...
  ctx.effect(
    ...
    "@cheeco/dsh-client-ui-plugin-manager: dictionaries"
  );
