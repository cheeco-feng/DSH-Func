...
var NAME = "@cheeco/dsh-client-ui-plugin-manager";
var ROUTE_PREFIX = "/pmgr";
var MARKER = "dsh-plugin-manager";
var DEFAULT_PROFILE = "web"; // fallback only; the real current profile is resolved below
...
/** resolveProfileName: 不再写死 web —— 优先用插件自己配置的 profile，否则从当前 dsh 进程的
 *  `--profile <name>` 启动参数解出**当前工作台**（与 cheeco 小功能的动态定位同理），最后回退到 web。 */
function resolveProfileName(config) {
  if (config && typeof config.profile === "string" && config.profile.trim()) return config.profile.trim();
  const args = process.argv.slice(2);
  const pi = args.indexOf("--profile");
  if (pi !== -1 && args[pi + 1] && /^[A-Za-z0-9_-]+$/.test(args[pi + 1])) return args[pi + 1];
