/** dsh-web-ui-patchPackagePanel host half — 一个最小 Cordis 插件（class 形状）。
 *  职责：
 *    - GET  /dsh-patch/config   -> 读取 @cheeco/setting/DSH-Patch-config.json
 *    - POST /dsh-patch/config   -> 写入 @cheeco/setting/DSH-Patch-config.json
 *
 *  与 cheeco-style 不同（按用户要求）：不使用 cheeco-config.json，而是新建
 *  DSH-Patch-config.json，放在 <home>/profiles/<p>/node_modules/@cheeco/setting/
...
const CONFIG_PATH = "/dsh-patch/config";
...
const CONFIG_FILENAME = "DSH-Patch-config.json";

/** 运行时配置文件：<@cheeco>/setting/DSH-Patch-config.json。 */
...
function renderConfigFile(v) {
	...
	return [
		"{",
		"  // 该设置页（DSH补丁包）在侧边栏的名字；留空用默认「DSH补丁包」",
		`  "label": ${s(v.label)},`,
...
/** 确保 DSH-Patch-config.json 存在且带 DSH 元数据（仅不存在时创建）。 */
...
export default class DshWebUiPatchPackagePanel {
	static name = "web-ui-patch-package-panel";
	...
	constructor(ctx, config) {
		...
		ctx.effect(() => {
			...
			return () => dispose();
		}, "dsh-patch-package: config route");
	}
