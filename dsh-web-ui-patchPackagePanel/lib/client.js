/** @cheeco/dsh-web-ui-patchPackagePanel client half（浏览器侧）。
 *
 *  在设置对话框侧边栏注册一个**新增槽位**「DSH补丁包」，放在「Cheeco的小功能」下方
 ...
 *
 *  配置文件：宿主端 /dsh-patch/config 读写的 @cheeco/setting/DSH-Patch-config.json
 *  （**不使用** cheeco-config.json）。 */
window.__ModuleLoader__.load({
  id: "@cheeco/dsh-web-ui-patchPackagePanel",
  factory: (require) => {
    var module = { exports: {} };
    ...
    (function () {
      if (typeof document === "undefined" || document.querySelector('style[data-plugin="dsh-web-ui-patch-package"]')) return;
      var css = ".dsh-sys{display:flex;flex-direction:column;gap:16px;}"
        + ".dsh-patch-card{box-sizing:border-box;background:var(--dsw-alias-bg-layer-2,#fff);border:1px solid var(--dsw-alias-border-l1,#e5e5e5);border-radius:12px;padding:16px 18px;box-shadow:0 1px 2px rgba(0,0,0,.04);}"
        + ".dsh-patch-card h3{margin:0 0 10px;font-size:16px;font-weight:600;color:var(--dsw-alias-label-primary,#1a1a1a);}"
        + ".dsh-patch-state{margin:0 0 10px;font-size:13px;line-height:20px;color:var(--dsw-alias-label-secondary,#666);}"
        + ".dsh-patch-actions{display:flex;flex-wrap:wrap;align-items:center;gap:8px;margin-top:4px;}"
        + ".dsh-patch-action{box-sizing:border-box;cursor:pointer;border:1px solid var(--dsw-alias-border-l2,#d9d9d9);background:var(--dsw-alias-bg-layer-3,#fff);color:var(--dsw-alias-label-primary,#1a1a1a);font-family:inherit;font-size:13px;line-height:20px;border-radius:8px;padding:6px 14px;transition:background .15s,border-color .15s,box-shadow .15s;}"
        + ".dsh-patch-action:hover{background:var(--dsw-alias-interactive-bg-hover,#f5f5f5);border-color:var(--dsw-alias-state-business-primary,#3498db);}"
        + ".dsh-patch-action:active{background:var(--dsw-alias-interactive-bg-active,#ececec);}"
        + ".dsh-patch-actions input{box-sizing:border-box;flex:1;min-width:180px;border:1px solid var(--dsw-alias-border-l2,#d9d9d9);border-radius:8px;padding:6px 12px;font-family:inherit;font-size:13px;line-height:20px;color:var(--dsw-alias-label-primary,#1a1a1a);background:var(--dsw-alias-bg-base,#fff);outline:none;}"
        + ".dsh-patch-actions input:focus{border-color:var(--dsw-alias-state-business-primary,#3498db);box-shadow:0 0 0 2px rgba(52,152,219,.15);}";
      var tag = document.createElement("style");
      tag.dataset.plugin = "dsh-web-ui-patch-package";
      tag.textContent = css;
      document.head.appendChild(tag);
    })();

    /** 本插件字典命名空间。 */
    const NS = "dsh-web-ui-patch-package";
    const zh = {
      "nav": "DSH补丁包"
    };
    const DEFAULT_LABEL = "DSH补丁包";

    /** 宿主端点：读/写 DSH-Patch-config.json。 */
    const CONFIG_ENDPOINT = "/dsh-patch/config";
    ...
    /** 合并 patch 并 POST 全量到宿主，写入 DSH-Patch-config.json（与 cheeco-style 相同，纯写配置）。 */
    ...
    function MaintenanceCard() {
      return react_jsx_runtime.jsx("div", {
        className: "dsh-patch-card",
        children: react_jsx_runtime.jsx("p", {
          className: "dsh-patch-state",
          children: "当前页面 正在维护中"
        })
      });
    }
    ...
    function RenameCard() {
      ...
      return react_jsx_runtime.jsx("div", {
        ...
        children: [
          ...
          react_jsx_runtime.jsx("p", { className: "dsh-patch-state", children: "给这个设置页在侧边栏的名字改名。" }),
          react_jsx_runtime.jsx("div", { className: "dsh-patch-actions", children: [
            ...
            react_jsx_runtime.jsx("button", { type: "button", className: "dsh-patch-action", onClick: save, children: "保存" })
          ] })
        ]
      });
    }
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => ctx.locale.register(NS, { zh, en: zh }), "dsh-web-ui-patch-package: dictionaries");
      const registerSection = () => ctx.slots.inject("settings.section", () => ctx.slots.register({
        name: "settings.section",
        id: "dsh-patch-package",
