/** dsh-web-ui-PluginPackagePanel host half — 一个最小 Cordis 插件（class 形状）。
 *  职责：
 *    - GET  /dsh-plugin/config   -> 读取 @cheeco/setting/DSH-Plugin-config.json
 *    - POST /dsh-plugin/config   -> 写入 @cheeco/setting/DSH-Plugin-config.json
 *
 *  与 cheeco-style 不同（按用户要求）：不使用 cheeco-config.json，而是新建
 *  DSH-Plugin-config.json，放在 <home>/profiles/<p>/node_modules/@cheeco/setting/
...
const CONFIG_PATH = "/dsh-plugin/config";
...
const CONFIG_FILENAME = "DSH-Plugin-config.json";

/** 运行时配置文件：<@cheeco>/setting/DSH-Plugin-config.json。 */
...
function renderConfigFile(v) {
	...
	return [
		"{",
		"  // 该设置页（DSH插件包）在侧边栏的名字；留空用默认「DSH插件包」",
		`  "label": ${s(v.label)},`,
...
/** 确保 DSH-Plugin-config.json 存在且带 DSH 元数据（仅不存在时创建）。 */
...
export default class DshWebUiPluginPackagePanel {
	static name = "web-ui-plugin-package-panel";
	...
	constructor(ctx, config) {
		...
		ctx.effect(() => {
			...
			return () => dispose();
		}, "dsh-plugin-package: config route");
	}
