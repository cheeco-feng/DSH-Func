...
一个 DSH 客户端插件：在 **设置 → 侧边栏** 新增一个槽位 **「DSH功能包」**（放在「Cheeco的小功能」下方），内页是 tab 结构。

## 当前内容
- 内页为 tab 结构（「功能表」「功能管理」「模型设置」「技能」「MCP管理」「面版管理」）：
  - **功能表**：占位卡片 **「当前页面 正在维护中」**（先做成空的）；
  - **功能管理**：功能开关（配置驱动，加/减开关只改 `DSH-Func-config.json` 的 `featureSwitches`）。当前包含：会话搜索功能、DSH功能命令、显示引用功能按钮、**显示监控面版**（`features.showMonitorPanel`，控制槽位窗口管理面板的「监控面版」tab 显隐）；
  - **面版管理**：唯一小卡片 **「面版改名」**——给这个设置页在侧边栏的名字改名（留空用默认「DSH功能包」）。
